...
namespace OpenRA.Mods.Common.Traits
{
	...
	public sealed class CoTBuildingEmitterInfo : PausableConditionalTraitInfo
	{
		...
		public readonly string Callsign = "OpenRA-Building";
		...
		public readonly int UpdateIntervalTicks = 150;

		[Desc("Maximum ticks between heartbeat sends; ensures updates even when stationary.")]
		public readonly int MaxIntervalTicks = 1500;
	...
	public sealed class CoTBuildingEmitter : PausableConditionalTrait<CoTBuildingEmitterInfo>, INotifyAddedToWorld, INotifyDamageStateChanged, INotifyKilled, ITick
	{
		readonly CoTBuildingEmitterInfo info;
			...
			: base(info)
		{
			...
			Log.Write("cot", string.Format(CultureInfo.InvariantCulture,
				"building-emitter init endpoint={0} callsign={1} type={2} updateTicks={3} maxTicks={4}",
		...
		string ResolveMilsymId(Actor self)
		{
		...
		void SendEvent(Actor self, double lat, double lon, string type, string callsign, int staleSeconds, string reason)
		{
			...
			var milsymId = ResolveMilsymId(self);
		...
		void INotifyKilled.Killed(Actor self, AttackInfo e)
		{
			...
			if (!TryGetLatLon(self, out var lat, out var lon))
				return;
			SendEvent(self, lat, lon, ResolveType(self), ResolveCallsign(self), info.StaleSeconds, "killed");
		}
		...
		protected override void TraitDisabled(Actor self)
		{
			...
			if (!TryGetLatLon(self, out var lat, out var lon))
				return;
			Log.Write("cot", "building-emitter disabled; sending stale update");
		...
		protected override void TraitEnabled(Actor self)
		{
			...
			if (!TryGetLatLon(self, out var lat, out var lon))
				return;
			Log.Write("cot", "building-emitter enabled; refreshing marker");
		...
		protected override void TraitResumed(Actor self)
		{
			...
			if (!TryGetLatLon(self, out var lat, out var lon))
				return;
			Log.Write("cot", "building-emitter resumed; refreshing marker");
		...
		protected override void TraitPaused(Actor self)
		{
			...
			if (!TryGetLatLon(self, out var lat, out var lon))
				return;
			Log.Write("cot", "building-emitter paused; sending stale update");
